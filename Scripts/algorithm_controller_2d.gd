...
@onready var algorithm_game: Node2D = $"../.."
@onready var algorithm_controller: Node = $"."
...
func _ready() -> void:
	...
	# Set process modes
	algorithm_game.process_mode = Node.PROCESS_MODE_INHERIT  # algorithm_game will inherit process mode
	self.process_mode = Node.PROCESS_MODE_ALWAYS  # algorithm_controller will always process
...
func _process(_delta):
	...
	while udp_socket.get_available_packet_count() > 0:
		...
		if not initial_message_received:
			...
			if message == "Hello from Algorithm":
